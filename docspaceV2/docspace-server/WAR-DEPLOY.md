...
- `DocspaceServerApplication` now supports servlet container startup
...
```bash
java -jar target/docspace-server.war
```

Deploy to external Tomcat:

1. Build the WAR package
2. Copy `target/docspace-server.war` to Tomcat `webapps/`
3. Start Tomcat
