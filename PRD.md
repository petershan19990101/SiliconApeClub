...
当前 docspaceV2 已经具备文档上传、目录、权限、版本、解析、人工校正、审计、发布、RAG 同步状态等能力，适合作为第一阶段的文档治理基础。但如果要支撑 AI 员工真正工作，仅有文档库和普通 RAG 不够。企业需要的是一套完整的知识层：
...
4. 为逐岗替换提供岗位知识包、权限边界、任务上下文和审批规则。
...
4. 从普通文档目录升级为岗位知识包、项目知识包、流程知识包、组织记忆。
...
- 知识热度与质量评估
- Wiki 与 RAG 一致性
- AI 员工 Knowledge Runtime API
- AI 员工反向沉淀 Wiki 流程
- 知识管理员与知识健康报告
- 每日知识静默巡检窗口
- 数据模型与技术架构建议
- 当前 DocSpace 的保留、升级与推倒原因

### 4.2 非本期范围

以下能力只预留接口，不在知识层 MVP 中完整实现：
...
- 按身份、岗位、任务权限读取知识。
- 启动任务时加载运行时上下文、岗位知识包和 must-read Wiki。
- 引用知识完成工作。
...
RAG Memory 层
  ...
  ↓
Knowledge Runtime API 层
  运行时上下文 / Wiki 结构化读取 / RAG 检索 / 引用日志 / 知识沉淀入口
  ↓
AI 员工调用与沉淀层
  岗位知识包加载 / 任务知识检索 / 组织记忆 / 项目记忆 / Wiki 草稿提案 / 知识反馈
  ↓
知识运营层
...
岗位知识包是某类人类岗位或 AI 员工岗位的默认知识范围、工作规则和审批边界。

### 8.6 知识健康

知识健康是对知识资产可用性、可信度、时效性、一致性、热度、风险的综合评估。

### 8.7 Knowledge Runtime API

Knowledge Runtime API 是面向 AI 员工的知识运行时接口层。它不是后台管理 API，而是 AI 员工在执行任务时加载身份、岗位、权限、Wiki 上下文、RAG 检索结果、引用日志和知识沉淀入口的统一通道。
...
AI 员工发起任务
  ↓
请求 Knowledge Runtime API 加载运行时上下文
  ↓
加载身份、岗位、部门、项目、任务上下文
  ↓
加载岗位知识包、must-read Wiki、默认检索 scope、权限边界
  ↓
读取必要 Wiki 页面结构化摘要
  ↓
根据任务生成一个或多个检索请求
...
返回可引用知识
  ↓
AI 基于 Wiki 上下文和 RAG 结果生成输出
  ↓
写入 citation log、usage event 和任务记忆
  ↓
根据任务结果判断是否产生知识反馈或 Wiki 知识提案
```
...
### 10.9 AI 员工 Knowledge Runtime API

知识层必须提供面向 AI 员工的运行时 API，避免 AI 员工直接耦合后台管理接口。
...
目标：让 DocSpace 从 RAG 状态占位升级为真实知识索引。
...
| RAG 黑盒 | AI 做错事无法定位原因 | RAG 调试台、引用回放、任务日志 |
...
当前 DocSpace 的工程基础应保留，但产品边界必须升级：从文档管理系统升级为企业知识资产平台；从 RAG 同步按钮升级为知识生产流水线；从人类文档权限升级为人类与 AI 员工统一权限；从不可见的向量索引升级为可观测、可巡检、可回放的 RAG Memory。
