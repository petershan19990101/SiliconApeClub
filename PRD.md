...
6. 管理台组织与人力中心作为公司组织、岗位、AI 员工属性、联系人关系、模型配置和岗位知识绑定的权威配置源。
7. 客户会员中心维护客户角色、可见部门、可见员工、可咨询和可派活权限。
8. Worker Platform 启动时把管理台组织和客户可见性投影到 `wp_*` 运行时表，客户端只读取 worker platform。
9. 前台接收任务后，按组织关系、部门职责、岗位知识包和 Skill 拆解派发。
10. 支持 markdown、受控 html、动态 form、artifact、task_status、org_route、employee_card 等多模态输出。
11. 建立长任务账本、事件流和 checkpoint，服务重启或停电后可按需求组恢复。
12. 支持任务结论沉淀为候选 Wiki，经审核发布后同步 RAG。
...
- AI 员工反向沉淀 Wiki 流程
- 组织与人力中心、客户会员中心、客户需求分组、聊天记录、业务前台、员工派活、组织关系和长任务恢复
- 知识管理员与知识健康报告
- 每日知识静默巡检窗口
- 数据模型与技术架构建议
...
- 为 AI 员工配置职责、技能、联系人关系、个人记忆策略、成本基线和专用模型。
- 为 AI 员工配置岗位知识。
...
- 上传 PDF、DOCX、PPTX、Markdown、TXT、图片等文件。
- 从企业系统同步文档。
