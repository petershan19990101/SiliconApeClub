...
- 后续新增 `siliconApeClub-worker-platform` 作为硅基俱乐部员工平台，对外提供 AI 员工服务能力、任务入口和运行期交互。
- 管理台负责“治理知识与配置员工”，员工平台负责“调用知识与交付服务”。
...
Knowledge Runtime API 层
  ...
  ↓
AI 员工调用与沉淀层
  岗位知识加载 / 任务知识检索 / 组织记忆 / 项目记忆 / Wiki 草稿提案 / 知识反馈
  ↓
知识运营层
...
加载身份、岗位、部门、项目、任务上下文
  ↓
加载岗位知识、must-read Wiki、默认检索 scope、权限边界
  ↓
读取必要 Wiki 页面结构化摘要
  ↓
根据任务生成一个或多个检索请求
...
AI 员工必须绑定一个或多个岗位知识对象。绑定后，Knowledge Runtime API 读取岗位知识下的 Wiki 页面集合、必读规则和默认检索范围，再生成 AI 员工任务启动上下文。
