...
- `siliconApeClub-worker-front` 是硅基俱乐部 AI 员工平台前端，对外提供客户入口、需求工作台、聊天记录、多模态输出和任务账本展示。
...
3. 按客户需求组管理历史需求、聊天记录、任务、附件和产出物。
4. 初始化“业务前台人员”角色，外部客户默认由业务前台接待。
5. 内部人员可按权限查看组织关系，对授权 AI 员工派活或咨询。
6. 管理台组织与人力中心作为公司组织、岗位、AI 员工属性、员工生命周期、联系人关系、模型配置、岗位知识绑定、考核规则和计量数据的权威配置源。
7. 客户会员中心维护客户角色、可见部门、可见员工、可咨询和可派活权限。
8. 技能仓库维护人类编写或 AI 员工总结出来的 Skill，必须审核通过后才能绑定到员工和投影到 worker platform。
9. Worker Platform 启动时把管理台组织、客户可见性、审核通过的技能和员工技能绑定投影到 `wp_*` 运行时表，客户端只读取 worker platform。
10. 前台接收任务后，按组织关系、部门职责、岗位知识包和 Skill 拆解派发。
11. 支持 markdown、受控 html、动态 form、artifact、task_status、org_route、employee_card 等多模态输出。
12. 建立长任务账本、事件流和 checkpoint，服务重启或停电后可按需求组恢复。
13. 支持任务结论沉淀为候选 Wiki，经审核发布后同步 RAG。
...
- AI 员工反向沉淀 Wiki 流程
- 组织与人力中心、技能仓库、客户会员中心、客户需求分组、聊天记录、业务前台、员工派活、组织关系、员工绩效计量和长任务恢复
- 知识管理员与知识健康报告
- 每日知识静默巡检窗口
- 数据模型与技术架构建议
...
- 管控高级技能开放范围，高级技能默认只面向顶级管理人员和授权员工。
...
LLM Wiki 层
  ...
  ↓
AI 员工平台
  客户登录 / 需求组 / 聊天记录 / 业务前台 / 组织派发 / 员工直派 / 长任务恢复 / 多模态输出
  ↓
内部知识运行时
...
需求组是客户历史需求管理的基本单位。一个需求组包含需求摘要、状态、参与员工、会话、任务、附件、产出物和知识沉淀候选。
...
目标：让客户和内部人员通过 AI 员工平台提交需求，并由业务前台按组织关系派发给 AI 员工团队。
