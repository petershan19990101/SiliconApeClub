...
`siliconApeClub-server` 是与 `siliconApeClub-front` 平级的后端项目，采用以下技术栈：
...
- Wiki 页面、知识同步任务、岗位知识管理、AI 员工、知识健康巡检
...
`siliconApeClub-server/docker-compose.yml` 仅保留基础中间件入口；完整服务编排请使用仓库根目录的 `docker-compose.yml`。
