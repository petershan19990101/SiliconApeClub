...
public class PositionPackageService {
    ...
    public List<Map<String, Object>> listPackages() {
        return jdbcTemplate.query(
                "SELECT p.id, p.code, p.name, p.description, p.position_code, p.default_scope_json, p.rules_json, p.status, " +
                        "p.created_by, p.created_at, p.updated_at, " +
                        "(SELECT COUNT(1) FROM ks_position_package_item i WHERE i.package_id = p.id) AS item_count " +
                        "FROM ks_position_package p WHERE p.status <> 'deleted' ORDER BY p.updated_at DESC",
    ...
    public Map<String, Object> getPackage(Long id) {
        Map<String, Object> result = jdbcTemplate.queryForObject(
                ...
                "FROM ks_position_package WHERE id = ? AND status <> 'deleted'",
    ...
    public Map<String, Object> submitReview(Long id) {
        jdbcTemplate.update("UPDATE ks_position_package SET status = 'pending_review', updated_at = ? WHERE id = ?",
    ...
    public Map<String, Object> reject(Long id) {
        jdbcTemplate.update("UPDATE ks_position_package SET status = 'rejected', updated_at = ? WHERE id = ?",
                LocalDateTime.now(), id);
        return getPackage(id);
    }

    public Map<String, Object> archive(Long id) {
        jdbcTemplate.update("UPDATE ks_position_package SET status = 'archived', updated_at = ? WHERE id = ?",
                LocalDateTime.now(), id);
        return getPackage(id);
    }
    ...
    public void delete(Long id) {
        ...
        jdbcTemplate.update("UPDATE ks_position_package SET status = 'deleted', updated_at = ? WHERE id = ?",
                LocalDateTime.now(), id);
    }
    ...
    public Map<String, Object> replaceItems(Long id, PositionPackageItemsRequest request) {
        ...
        jdbcTemplate.update("UPDATE ks_position_package SET status = 'draft', updated_at = ? WHERE id = ?",
                LocalDateTime.now(), id);
        return getPackage(id);
    }
