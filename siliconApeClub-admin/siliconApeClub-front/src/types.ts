...
export interface WikiStructureGroup {
  type: 'department' | 'pageType' | 'status';
  value: string;
  label: string;
  count: number;
  children?: WikiStructureGroup[];
}
...
export interface WikiRelation {
  ...
  targetPageId: string;
  targetTitle?: string;
  relationType: 'references' | 'depends_on' | 'related_to' | 'supersedes' | 'duplicated_with';
...
export interface PositionPackage {
  ...
  status: string;
  itemCount?: number;
  items?: PositionKnowledgeItem[];
}

export interface PositionKnowledgeItem {
  id?: string;
  packageId?: string;
  itemType: string;
  itemId: string;
...
export interface RagAclBinding {
  id: string;
  policyId: string;
  policyName?: string;
