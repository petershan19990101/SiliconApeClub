...
- 加载岗位知识包、must-read Wiki 和默认检索范围。
...
| Doc Service | 保留现有 DocSpace 后端 | 文档上传、目录、版本、解析产物、源文件管理 | 可共享主库 |
...
| Position Package Service | 可与 Wiki 合并 | 岗位知识包、AI 员工绑定、默认检索范围 | 共享主库 |
...
现有 docspaceV2 的文档管理能力可作为该服务的基础。
...
knowledge-runtime-service 加载 AI 身份、岗位、项目、任务上下文
  ↓
加载岗位知识包 runtime profile、must-read Wiki、默认检索 scope
  ↓
读取必要 Wiki 页面结构化摘要
  ↓
...
| 图谱可视化 | 暂缓 | AntV G6 / Cytoscape | 知识关系后期使用 |
...
当前 docspaceV2 使用 Java 8 和 Spring Boot 2.2.x。短期可以继续迭代，但长期企业级产品建议升级到 Java 17 + Spring Boot 3.x，原因是安全维护、依赖生态、性能和云原生支持都更好。
...
如果允许调整当前 DocSpace 技术底座，知识层主库优先 PostgreSQL。原因是 PostgreSQL 可以同时承载结构化知识、JSONB、全文检索和 pgvector，MVP 架构更轻。
...
```text
MVP：DocSpace 模块化单体 + Pipeline Worker + Retrieval Service
